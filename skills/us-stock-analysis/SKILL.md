...
### Step 0: Real-Time Stock Price (Always Try First)

Before any analysis, attempt to fetch the real-time stock price via the Intellectia API (no API key required):

```bash
curl -s --compressed "https://api.intellectia.ai/gateway/v1/stock/quote?symbol={TICKER}" 2>/dev/null
```

If that endpoint is unavailable, try the screener to find the ticker:
```bash
curl -s --compressed "https://api.intellectia.ai/gateway/v1/stock/screener-list?symbol_type=0&period_type=0&trend_type=0&page=1&size=100" 2>/dev/null | python3 -c "
import json,sys
d=json.load(sys.stdin)
rows=d.get('data',{}).get('list',[])
match=[r for r in rows if r.get('symbol','').upper()=='{TICKER}']
if match:
    r=match[0]
    print(f'Price: \${r[\"price\"]}  Change: {r[\"change_ratio\"]:+.2f}%  AI Probability: {r[\"probability\"]}%  Predicted Return: {r[\"profit\"]:+.1f}%')
else:
    print('not_found')
"
```

**Rule:** If real-time price is found → include it prominently at the top of the report. If not found (API error, ticker not in screener) → skip price section and proceed with the rest of the analysis using web search data. Never block the analysis waiting for a price.
