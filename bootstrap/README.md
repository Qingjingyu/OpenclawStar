...
## One-Click: Activate CEO + CTO + RD Sandbox (Recommended for Safe Evolution)

Use this when you want:
- CEO as dialogue/acceptance entry
- CTO as production execution owner
- a third isolated RD engineer instance for sandbox testing only

```bash
git clone -b master git@github.com:Qingjingyu/Yoyoo.git
cd Yoyoo
export MINIMAX_API_KEY='your_key'
bash Yoyoo/project/bootstrap/activate_ceo_cto_rd_sandbox.sh
```

Defaults:
- CEO: `/root/.openclaw` on `:18789`
- CTO: `/root/.openclaw-cto` on `:18794`
- RD Sandbox: `/srv/yoyoo/rd-sandbox/state` on `:18793`

Behavior:
- CEO/CTO share core team memory (`MEMORY.md + memory/`) by default.
- RD sandbox is isolated by default (no shared production memory, no Feishu expectation).
- Acceptance check scans `ceo cto rd-engineer`.

Optional:
- `YOYOO_RUN_ACCEPTANCE=0` to skip acceptance check.
- `YOYOO_TEAM_SHARED_MEMORY=0` to disable CEO/CTO shared memory wiring.
- `YOYOO_RD_SANDBOX_KEY`, `YOYOO_RD_SANDBOX_HOME`, `YOYOO_RD_SANDBOX_PORT` to customize sandbox instance.
...
- `activate_ceo_cto_rd_sandbox.sh`: one-step activation for CEO + CTO + isolated RD sandbox employee.
